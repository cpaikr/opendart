...
/// Inputs for logical operation `DS003-2019017`.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CompaniesKeyAccountsInput {
    company_codes: Vec<String>,
    business_year: String,
    report_code: String,
}

impl CompaniesKeyAccountsInput {
    /// Creates inputs from all required source-shaped values.
    #[must_use]
    pub fn new(company_codes: Vec<String>, business_year: String, report_code: String) -> Self {
        Self {
            company_codes,
            business_year,
            report_code,
    ...
    /// Prepares physical operation `get_fnlttMultiAcnt_json`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_json(
        &self,
    ) -> Result<PreparedRequest<CompaniesKeyAccountsJsonResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_fnlttMultiAcnt_json", "DS003-2019017");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            RequestParts::structured_json("/api/fnlttMultiAcnt.json", operation, &parameters),
            decode_companies_key_accounts_json_response,
        ))
    }

    /// Prepares physical operation `get_fnlttMultiAcnt_xml`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_xml(
        &self,
    ) -> Result<PreparedRequest<CompaniesKeyAccountsXmlResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_fnlttMultiAcnt_xml", "DS003-2019017");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            ...
            decode_companies_key_accounts_xml_response,
        ))
    }
}
...
pub struct CompaniesKeyAccount<'a> {
    source: &'a SourceValue,
}

impl<'a> CompaniesKeyAccount<'a> {
...
/// Opaque response for physical operation `get_fnlttMultiAcnt_json`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct CompaniesKeyAccountsJsonResponse {
    source: SourceValue,
}

impl CompaniesKeyAccountsJsonResponse {
    ...
    pub fn items(&self) -> impl Iterator<Item = CompaniesKeyAccount<'_>> + '_ {
        items(&self.source, "list", false).map(CompaniesKeyAccount::new)
    }
}

fn decode_companies_key_accounts_json_response(
    source: SourceValue,
) -> Result<CompaniesKeyAccountsJsonResponse, ResponseDecodeError> {
    decode_companies_key_accounts_json_response_root(&source, "$".to_owned())?;
    Ok(CompaniesKeyAccountsJsonResponse { source })
}

fn decode_companies_key_accounts_json_response_root(
    value: &SourceValue,
    path: String,
    ...
    object.optional(
        "list",
        decode_companies_key_accounts_json_response_root_list,
    )?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_companies_key_accounts_json_response_root_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_array(
        value,
        path,
        decode_companies_key_accounts_json_response_root_list_item,
    )?;
    Ok(())
}

fn decode_companies_key_accounts_json_response_root_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
...
/// Opaque response for physical operation `get_fnlttMultiAcnt_xml`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct CompaniesKeyAccountsXmlResponse {
    source: SourceValue,
}

impl CompaniesKeyAccountsXmlResponse {
    /// Returns the complete normalized source object.
    #[must_use]
    pub const fn source(&self) -> &SourceValue {
        &self.source
    ...
    /// Iterates the reviewed `$.list[]` item views.
    pub fn items(&self) -> impl Iterator<Item = CompaniesKeyAccount<'_>> + '_ {
        items(&self.source, "list", true).map(CompaniesKeyAccount::new)
    }
}

fn decode_companies_key_accounts_xml_response(
    source: SourceValue,
) -> Result<CompaniesKeyAccountsXmlResponse, ResponseDecodeError> {
    decode_companies_key_accounts_xml_response_root(&source, "$".to_owned())?;
    Ok(CompaniesKeyAccountsXmlResponse { source })
}

fn decode_companies_key_accounts_xml_response_root(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    let object = ObjectDecoder::new_xml(value, path)?;
    object.optional("list", decode_companies_key_accounts_xml_response_root_list)?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_companies_key_accounts_xml_response_root_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_xml_array(
        value,
        path,
        decode_companies_key_accounts_xml_response_root_list_item,
    )?;
    Ok(())
}

fn decode_companies_key_accounts_xml_response_root_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
