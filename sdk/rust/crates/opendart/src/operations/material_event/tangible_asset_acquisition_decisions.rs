...
/// Inputs for logical operation `DS005-2020044`.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TangibleAssetAcquisitionDecisionsInput {
    company_code: String,
    start_date: String,
    end_date: String,
}

impl TangibleAssetAcquisitionDecisionsInput {
    ...
    /// Prepares physical operation `get_tgastInhDecsn_json`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_json(
        &self,
    ) -> Result<PreparedRequest<TangibleAssetAcquisitionDecisionsJsonResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_tgastInhDecsn_json", "DS005-2020044");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            RequestParts::structured_json("/api/tgastInhDecsn.json", operation, &parameters),
            decode_tangible_asset_acquisition_decisions_json_response,
        ))
    }

    /// Prepares physical operation `get_tgastInhDecsn_xml`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_xml(
        &self,
    ) -> Result<PreparedRequest<TangibleAssetAcquisitionDecisionsXmlResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_tgastInhDecsn_xml", "DS005-2020044");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            RequestParts::structured_xml(
                "/api/tgastInhDecsn.xml",
                operation,
                &parameters,
                "result",
            ),
            decode_tangible_asset_acquisition_decisions_xml_response,
        ))
    }
}
...
pub struct TangibleAssetAcquisitionDecision<'a> {
    source: &'a SourceValue,
}

impl<'a> TangibleAssetAcquisitionDecision<'a> {
    ...
    /// Returns source field `inh_prd_rgs_prd` when present.
    #[must_use]
    pub fn registration_date(&self) -> Option<&SourceValue> {
        self.source.get("inh_prd_rgs_prd")
    }
...
/// Opaque response for physical operation `get_tgastInhDecsn_json`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct TangibleAssetAcquisitionDecisionsJsonResponse {
    source: SourceValue,
}

impl TangibleAssetAcquisitionDecisionsJsonResponse {
    ...
    pub fn items(&self) -> impl Iterator<Item = TangibleAssetAcquisitionDecision<'_>> + '_ {
        items(&self.source, "list", false).map(TangibleAssetAcquisitionDecision::new)
    }
}

fn decode_tangible_asset_acquisition_decisions_json_response(
    source: SourceValue,
) -> Result<TangibleAssetAcquisitionDecisionsJsonResponse, ResponseDecodeError> {
    decode_tangible_asset_acquisition_decisions_json_response_root(&source, "$".to_owned())?;
    Ok(TangibleAssetAcquisitionDecisionsJsonResponse { source })
}

fn decode_tangible_asset_acquisition_decisions_json_response_root(
    value: &SourceValue,
    path: String,
    ...
    object.optional(
        "list",
        decode_tangible_asset_acquisition_decisions_json_response_root_list,
    )?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_tangible_asset_acquisition_decisions_json_response_root_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_array(
        value,
        path,
        decode_tangible_asset_acquisition_decisions_json_response_root_list_item,
    )?;
    Ok(())
}

fn decode_tangible_asset_acquisition_decisions_json_response_root_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
...
/// Opaque response for physical operation `get_tgastInhDecsn_xml`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct TangibleAssetAcquisitionDecisionsXmlResponse {
    source: SourceValue,
}

impl TangibleAssetAcquisitionDecisionsXmlResponse {
    /// Returns the complete normalized source object.
    #[must_use]
    pub const fn source(&self) -> &SourceValue {
        &self.source
    ...
    /// Iterates the reviewed `$.list[]` item views.
    pub fn items(&self) -> impl Iterator<Item = TangibleAssetAcquisitionDecision<'_>> + '_ {
        items(&self.source, "list", true).map(TangibleAssetAcquisitionDecision::new)
    }
}

fn decode_tangible_asset_acquisition_decisions_xml_response(
    source: SourceValue,
) -> Result<TangibleAssetAcquisitionDecisionsXmlResponse, ResponseDecodeError> {
    decode_tangible_asset_acquisition_decisions_xml_response_root(&source, "$".to_owned())?;
    Ok(TangibleAssetAcquisitionDecisionsXmlResponse { source })
}

fn decode_tangible_asset_acquisition_decisions_xml_response_root(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    ...
    object.optional(
        "list",
        decode_tangible_asset_acquisition_decisions_xml_response_root_list,
    )?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_tangible_asset_acquisition_decisions_xml_response_root_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_xml_array(
        value,
        path,
        decode_tangible_asset_acquisition_decisions_xml_response_root_list_item,
    )?;
    Ok(())
}

fn decode_tangible_asset_acquisition_decisions_xml_response_root_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
