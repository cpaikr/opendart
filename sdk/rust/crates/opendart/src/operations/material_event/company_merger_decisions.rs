...
/// Inputs for logical operation `DS005-2020050`.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CompanyMergerDecisionsInput {
    company_code: String,
    start_date: String,
    end_date: String,
}

impl CompanyMergerDecisionsInput {
    ...
    /// Prepares physical operation `get_cmpMgDecsn_json`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_json(
        &self,
    ) -> Result<PreparedRequest<CompanyMergerDecisionsJsonResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_cmpMgDecsn_json", "DS005-2020050");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            RequestParts::structured_json("/api/cmpMgDecsn.json", operation, &parameters),
            decode_company_merger_decisions_json_response,
        ))
    }

    /// Prepares physical operation `get_cmpMgDecsn_xml`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_xml(
        &self,
    ) -> Result<PreparedRequest<CompanyMergerDecisionsXmlResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_cmpMgDecsn_xml", "DS005-2020050");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            RequestParts::structured_xml("/api/cmpMgDecsn.xml", operation, &parameters, "result"),
            decode_company_merger_decisions_xml_response,
        ))
    }
}
...
pub struct CompanyMergerDecision<'a> {
    source: &'a SourceValue,
}

impl<'a> CompanyMergerDecision<'a> {
    ...
    #[must_use]
    pub fn new_merger_company_financials_as_of(&self) -> Option<&SourceValue> {
        self.source.get("ffdtl_std")
    }

    /// Returns source field `ffdtl_tast` when present.
    #[must_use]
    pub fn new_merger_company_total_assets(&self) -> Option<&SourceValue> {
        self.source.get("ffdtl_tast")
    }

    /// Returns source field `ffdtl_tdbt` when present.
    #[must_use]
    pub fn new_merger_company_total_liabilities(&self) -> Option<&SourceValue> {
        self.source.get("ffdtl_tdbt")
    }

    /// Returns source field `ffdtl_teqt` when present.
    #[must_use]
    pub fn new_merger_company_total_equity(&self) -> Option<&SourceValue> {
        self.source.get("ffdtl_teqt")
    }

    /// Returns source field `mg_mth` when present.
    #[must_use]
    pub fn merger_method(&self) -> Option<&SourceValue> {
        self.source.get("mg_mth")
    }

    /// Returns source field `mg_pp` when present.
    #[must_use]
    pub fn merger_purpose(&self) -> Option<&SourceValue> {
        self.source.get("mg_pp")
    }

    /// Returns source field `mg_rt` when present.
    #[must_use]
    pub fn merger_ratio(&self) -> Option<&SourceValue> {
        self.source.get("mg_rt")
    }

    /// Returns source field `mg_rt_bs` when present.
    #[must_use]
    pub fn merger_ratio_basis(&self) -> Option<&SourceValue> {
        self.source.get("mg_rt_bs")
    }
    ...
    /// Returns source field `mgnstk_cstk_cnt` when present.
    #[must_use]
    pub fn new_class_shares_from_merger(&self) -> Option<&SourceValue> {
        self.source.get("mgnstk_cstk_cnt")
    }

    /// Returns source field `mgnstk_ostk_cnt` when present.
    #[must_use]
    pub fn new_common_shares_from_merger(&self) -> Option<&SourceValue> {
        self.source.get("mgnstk_ostk_cnt")
    }
    ...
    /// Returns source field `mgsc_aprskh_expd_bgd` when present.
    #[must_use]
    pub fn merger_appraisal_rights_exercise_start_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_aprskh_expd_bgd")
    }

    /// Returns source field `mgsc_aprskh_expd_edd` when present.
    #[must_use]
    pub fn merger_appraisal_rights_exercise_end_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_aprskh_expd_edd")
    }

    /// Returns source field `mgsc_cdobprpd_bgd` when present.
    #[must_use]
    pub fn merger_creditor_objection_start_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_cdobprpd_bgd")
    }

    /// Returns source field `mgsc_cdobprpd_edd` when present.
    #[must_use]
    pub fn merger_creditor_objection_end_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_cdobprpd_edd")
    }

    /// Returns source field `mgsc_ergmd` when present.
    #[must_use]
    pub fn merger_closing_report_meeting_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_ergmd")
    }

    /// Returns source field `mgsc_gmtsck_prd` when present.
    #[must_use]
    pub fn planned_merger_shareholders_meeting_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_gmtsck_prd")
    }

    /// Returns source field `mgsc_mgctrd` when present.
    #[must_use]
    pub fn merger_contract_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_mgctrd")
    }

    /// Returns source field `mgsc_mgdt` when present.
    #[must_use]
    pub fn merger_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_mgdt")
    }

    /// Returns source field `mgsc_mgop_rcpd_bgd` when present.
    #[must_use]
    pub fn merger_dissent_notice_period_start_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_mgop_rcpd_bgd")
    }

    /// Returns source field `mgsc_mgop_rcpd_edd` when present.
    #[must_use]
    pub fn merger_dissent_notice_period_end_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_mgop_rcpd_edd")
    }

    /// Returns source field `mgsc_mgrgsprd` when present.
    #[must_use]
    pub fn planned_merger_registration_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_mgrgsprd")
    }

    /// Returns source field `mgsc_nstkdlprd` when present.
    #[must_use]
    pub fn planned_merger_new_share_certificate_delivery_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_nstkdlprd")
    }

    /// Returns source field `mgsc_nstklstprd` when present.
    #[must_use]
    pub fn planned_merger_new_share_listing_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_nstklstprd")
    }

    /// Returns source field `mgsc_osprpd_bgd` when present.
    #[must_use]
    pub fn merger_old_share_certificate_submission_start_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_osprpd_bgd")
    }

    /// Returns source field `mgsc_osprpd_edd` when present.
    #[must_use]
    pub fn merger_old_share_certificate_submission_end_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_osprpd_edd")
    }

    /// Returns source field `mgsc_shclspd_bgd` when present.
    #[must_use]
    pub fn merger_shareholder_registry_closure_start_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_shclspd_bgd")
    }

    /// Returns source field `mgsc_shclspd_edd` when present.
    #[must_use]
    pub fn merger_shareholder_registry_closure_end_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_shclspd_edd")
    }

    /// Returns source field `mgsc_shddstd` when present.
    #[must_use]
    pub fn merger_shareholder_record_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_shddstd")
    }

    /// Returns source field `mgsc_trspprpd_bgd` when present.
    #[must_use]
    pub fn merger_trading_suspension_start_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_trspprpd_bgd")
    }

    /// Returns source field `mgsc_trspprpd_edd` when present.
    #[must_use]
    pub fn merger_trading_suspension_end_date(&self) -> Option<&SourceValue> {
        self.source.get("mgsc_trspprpd_edd")
    }
    ...
    /// Returns source field `nmgcmp_nbsn_rsl` when present.
    #[must_use]
    pub fn new_merger_company_new_business_segment_prior_year_revenue(
        &self,
    ) -> Option<&SourceValue> {
        self.source.get("nmgcmp_nbsn_rsl")
    }
...
/// Opaque response for physical operation `get_cmpMgDecsn_json`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct CompanyMergerDecisionsJsonResponse {
    source: SourceValue,
}

impl CompanyMergerDecisionsJsonResponse {
    ...
    pub fn items(&self) -> impl Iterator<Item = CompanyMergerDecision<'_>> + '_ {
        items(&self.source, "list", false).map(CompanyMergerDecision::new)
    }
}

fn decode_company_merger_decisions_json_response(
    source: SourceValue,
) -> Result<CompanyMergerDecisionsJsonResponse, ResponseDecodeError> {
    decode_company_merger_decisions_json_response_root(&source, "$".to_owned())?;
    Ok(CompanyMergerDecisionsJsonResponse { source })
}

fn decode_company_merger_decisions_json_response_root(
    value: &SourceValue,
    path: String,
    ...
    object.optional(
        "list",
        decode_company_merger_decisions_json_response_root_list,
    )?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_company_merger_decisions_json_response_root_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_array(
        value,
        path,
        decode_company_merger_decisions_json_response_root_list_item,
    )?;
    Ok(())
}

fn decode_company_merger_decisions_json_response_root_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
...
/// Opaque response for physical operation `get_cmpMgDecsn_xml`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct CompanyMergerDecisionsXmlResponse {
    source: SourceValue,
}

impl CompanyMergerDecisionsXmlResponse {
    /// Returns the complete normalized source object.
    #[must_use]
    pub const fn source(&self) -> &SourceValue {
        &self.source
    ...
    /// Iterates the reviewed `$.list[]` item views.
    pub fn items(&self) -> impl Iterator<Item = CompanyMergerDecision<'_>> + '_ {
        items(&self.source, "list", true).map(CompanyMergerDecision::new)
    }
}

fn decode_company_merger_decisions_xml_response(
    source: SourceValue,
) -> Result<CompanyMergerDecisionsXmlResponse, ResponseDecodeError> {
    decode_company_merger_decisions_xml_response_root(&source, "$".to_owned())?;
    Ok(CompanyMergerDecisionsXmlResponse { source })
}

fn decode_company_merger_decisions_xml_response_root(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    ...
    object.optional(
        "list",
        decode_company_merger_decisions_xml_response_root_list,
    )?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_company_merger_decisions_xml_response_root_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_xml_array(
        value,
        path,
        decode_company_merger_decisions_xml_response_root_list_item,
    )?;
    Ok(())
}

fn decode_company_merger_decisions_xml_response_root_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
