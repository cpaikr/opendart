...
/// Inputs for logical operation `DS005-2020043`.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct BusinessTransferDecisionsInput {
    company_code: String,
    start_date: String,
    end_date: String,
}

impl BusinessTransferDecisionsInput {
    ...
    /// Prepares physical operation `get_bsnTrfDecsn_json`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_json(
        &self,
    ) -> Result<PreparedRequest<BusinessTransferDecisionsJsonResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_bsnTrfDecsn_json", "DS005-2020043");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            RequestParts::structured_json("/api/bsnTrfDecsn.json", operation, &parameters),
            decode_business_transfer_decisions_json_response,
        ))
    }

    /// Prepares physical operation `get_bsnTrfDecsn_xml`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_xml(
        &self,
    ) -> Result<PreparedRequest<BusinessTransferDecisionsXmlResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_bsnTrfDecsn_xml", "DS005-2020043");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            RequestParts::structured_xml("/api/bsnTrfDecsn.xml", operation, &parameters, "result"),
            decode_business_transfer_decisions_xml_response,
        ))
    }
}
...
pub struct BusinessTransferDecision<'a> {
    source: &'a SourceValue,
}

impl<'a> BusinessTransferDecision<'a> {
    ...
    /// Returns source field `ast_trf_bsn` when present.
    #[must_use]
    pub fn target_business_assets(&self) -> Option<&SourceValue> {
        self.source.get("ast_trf_bsn")
    }
    ...
    /// Returns source field `sl_trf_bsn` when present.
    #[must_use]
    pub fn target_business_revenue(&self) -> Option<&SourceValue> {
        self.source.get("sl_trf_bsn")
    }
    ...
    /// Returns source field `trf_bsn` when present.
    #[must_use]
    pub fn transferred_business(&self) -> Option<&SourceValue> {
        self.source.get("trf_bsn")
    }

    /// Returns source field `trf_bsn_mc` when present.
    #[must_use]
    pub fn transferred_business_details(&self) -> Option<&SourceValue> {
        self.source.get("trf_bsn_mc")
    }
    ...
    /// Returns source field `trf_prc` when present.
    #[must_use]
    pub fn transfer_price(&self) -> Option<&SourceValue> {
        self.source.get("trf_prc")
    }
    ...
    /// Returns source field `trf_pym` when present.
    #[must_use]
    pub fn transfer_payment_terms(&self) -> Option<&SourceValue> {
        self.source.get("trf_pym")
    }
}

/// Opaque response for physical operation `get_bsnTrfDecsn_json`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct BusinessTransferDecisionsJsonResponse {
    source: SourceValue,
}

impl BusinessTransferDecisionsJsonResponse {
    ...
    pub fn items(&self) -> impl Iterator<Item = BusinessTransferDecision<'_>> + '_ {
        items(&self.source, "list", false).map(BusinessTransferDecision::new)
    }
}

fn decode_business_transfer_decisions_json_response(
    source: SourceValue,
) -> Result<BusinessTransferDecisionsJsonResponse, ResponseDecodeError> {
    decode_business_transfer_decisions_json_response_root(&source, "$".to_owned())?;
    Ok(BusinessTransferDecisionsJsonResponse { source })
}

fn decode_business_transfer_decisions_json_response_root(
    value: &SourceValue,
    path: String,
    ...
    object.optional(
        "list",
        decode_business_transfer_decisions_json_response_root_list,
    )?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_business_transfer_decisions_json_response_root_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_array(
        value,
        path,
        decode_business_transfer_decisions_json_response_root_list_item,
    )?;
    Ok(())
}

fn decode_business_transfer_decisions_json_response_root_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
...
/// Opaque response for physical operation `get_bsnTrfDecsn_xml`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct BusinessTransferDecisionsXmlResponse {
    source: SourceValue,
}

impl BusinessTransferDecisionsXmlResponse {
    /// Returns the complete normalized source object.
    #[must_use]
    pub const fn source(&self) -> &SourceValue {
        &self.source
    ...
    /// Iterates the reviewed `$.list[]` item views.
    pub fn items(&self) -> impl Iterator<Item = BusinessTransferDecision<'_>> + '_ {
        items(&self.source, "list", true).map(BusinessTransferDecision::new)
    }
}

fn decode_business_transfer_decisions_xml_response(
    source: SourceValue,
) -> Result<BusinessTransferDecisionsXmlResponse, ResponseDecodeError> {
    decode_business_transfer_decisions_xml_response_root(&source, "$".to_owned())?;
    Ok(BusinessTransferDecisionsXmlResponse { source })
}

fn decode_business_transfer_decisions_xml_response_root(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    ...
    object.optional(
        "list",
        decode_business_transfer_decisions_xml_response_root_list,
    )?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_business_transfer_decisions_xml_response_root_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_xml_array(
        value,
        path,
        decode_business_transfer_decisions_xml_response_root_list_item,
    )?;
    Ok(())
}

fn decode_business_transfer_decisions_xml_response_root_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
