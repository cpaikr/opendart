...
/// Inputs for logical operation `DS005-2020035`.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ExchangeableBondIssuanceDecisionsInput {
    company_code: String,
    start_date: String,
    end_date: String,
}

impl ExchangeableBondIssuanceDecisionsInput {
    ...
    /// Prepares physical operation `get_exbdIsDecsn_json`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_json(
        &self,
    ) -> Result<PreparedRequest<ExchangeableBondIssuanceDecisionsJsonResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_exbdIsDecsn_json", "DS005-2020035");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            RequestParts::structured_json("/api/exbdIsDecsn.json", operation, &parameters),
            decode_exchangeable_bond_issuance_decisions_json_response,
        ))
    }

    /// Prepares physical operation `get_exbdIsDecsn_xml`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_xml(
        &self,
    ) -> Result<PreparedRequest<ExchangeableBondIssuanceDecisionsXmlResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_exbdIsDecsn_xml", "DS005-2020035");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            RequestParts::structured_xml("/api/exbdIsDecsn.xml", operation, &parameters, "result"),
            decode_exchangeable_bond_issuance_decisions_xml_response,
        ))
    }
}
...
pub struct ExchangeableBondIssuanceDecision<'a> {
    source: &'a SourceValue,
}

impl<'a> ExchangeableBondIssuanceDecision<'a> {
    ...
    #[must_use]
    pub fn exchange_price_per_share(&self) -> Option<&SourceValue> {
        self.source.get("ex_prc")
    }

    /// Returns source field `ex_prc_dmth` when present.
    #[must_use]
    pub fn exchange_price_determination_method(&self) -> Option<&SourceValue> {
        self.source.get("ex_prc_dmth")
    }

    /// Returns source field `ex_rt` when present.
    #[must_use]
    pub fn exchange_ratio(&self) -> Option<&SourceValue> {
    ...
    /// Returns source field `exrqpd_bgd` when present.
    #[must_use]
    pub fn exchange_request_start_date(&self) -> Option<&SourceValue> {
        self.source.get("exrqpd_bgd")
    }

    /// Returns source field `exrqpd_edd` when present.
    #[must_use]
    pub fn exchange_request_end_date(&self) -> Option<&SourceValue> {
        self.source.get("exrqpd_edd")
    }

    /// Returns source field `extg` when present.
    #[must_use]
    pub fn exchange_target_share_type(&self) -> Option<&SourceValue> {
        self.source.get("extg")
    }

    /// Returns source field `extg_stkcnt` when present.
    #[must_use]
    pub fn exchange_target_share_count(&self) -> Option<&SourceValue> {
        self.source.get("extg_stkcnt")
    }

    /// Returns source field `extg_tisstk_vs` when present.
    #[must_use]
    pub fn exchange_target_shares_to_total_ratio(&self) -> Option<&SourceValue> {
        self.source.get("extg_tisstk_vs")
    }
...
/// Opaque response for physical operation `get_exbdIsDecsn_json`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct ExchangeableBondIssuanceDecisionsJsonResponse {
    source: SourceValue,
}

impl ExchangeableBondIssuanceDecisionsJsonResponse {
    ...
    pub fn items(&self) -> impl Iterator<Item = ExchangeableBondIssuanceDecision<'_>> + '_ {
        items(&self.source, "list", false).map(ExchangeableBondIssuanceDecision::new)
    }
}

fn decode_exchangeable_bond_issuance_decisions_json_response(
    source: SourceValue,
) -> Result<ExchangeableBondIssuanceDecisionsJsonResponse, ResponseDecodeError> {
    decode_exchangeable_bond_issuance_decisions_json_response_root(&source, "$".to_owned())?;
    Ok(ExchangeableBondIssuanceDecisionsJsonResponse { source })
}

fn decode_exchangeable_bond_issuance_decisions_json_response_root(
    value: &SourceValue,
    path: String,
    ...
    object.optional(
        "list",
        decode_exchangeable_bond_issuance_decisions_json_response_root_list,
    )?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_exchangeable_bond_issuance_decisions_json_response_root_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_array(
        value,
        path,
        decode_exchangeable_bond_issuance_decisions_json_response_root_list_item,
    )?;
    Ok(())
}

fn decode_exchangeable_bond_issuance_decisions_json_response_root_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
...
/// Opaque response for physical operation `get_exbdIsDecsn_xml`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct ExchangeableBondIssuanceDecisionsXmlResponse {
    source: SourceValue,
}

impl ExchangeableBondIssuanceDecisionsXmlResponse {
    /// Returns the complete normalized source object.
    #[must_use]
    pub const fn source(&self) -> &SourceValue {
        &self.source
    ...
    /// Iterates the reviewed `$.list[]` item views.
    pub fn items(&self) -> impl Iterator<Item = ExchangeableBondIssuanceDecision<'_>> + '_ {
        items(&self.source, "list", true).map(ExchangeableBondIssuanceDecision::new)
    }
}

fn decode_exchangeable_bond_issuance_decisions_xml_response(
    source: SourceValue,
) -> Result<ExchangeableBondIssuanceDecisionsXmlResponse, ResponseDecodeError> {
    decode_exchangeable_bond_issuance_decisions_xml_response_root(&source, "$".to_owned())?;
    Ok(ExchangeableBondIssuanceDecisionsXmlResponse { source })
}

fn decode_exchangeable_bond_issuance_decisions_xml_response_root(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    ...
    object.optional(
        "list",
        decode_exchangeable_bond_issuance_decisions_xml_response_root_list,
    )?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_exchangeable_bond_issuance_decisions_xml_response_root_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_xml_array(
        value,
        path,
        decode_exchangeable_bond_issuance_decisions_xml_response_root_list_item,
    )?;
    Ok(())
}

fn decode_exchangeable_bond_issuance_decisions_xml_response_root_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
