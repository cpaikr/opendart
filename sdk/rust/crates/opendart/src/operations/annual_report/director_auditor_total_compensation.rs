...
/// Inputs for logical operation `DS002-2019013`.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DirectorAuditorTotalCompensationInput {
    company_code: String,
    business_year: String,
    report_code: String,
}

impl DirectorAuditorTotalCompensationInput {
    ...
    /// Prepares physical operation `get_hmvAuditAllSttus_json`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_json(
        &self,
    ) -> Result<PreparedRequest<DirectorAuditorTotalCompensationJsonResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_hmvAuditAllSttus_json", "DS002-2019013");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            RequestParts::structured_json("/api/hmvAuditAllSttus.json", operation, &parameters),
            decode_director_auditor_total_compensation_json_response,
        ))
    }

    /// Prepares physical operation `get_hmvAuditAllSttus_xml`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_xml(
        &self,
    ) -> Result<PreparedRequest<DirectorAuditorTotalCompensationXmlResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_hmvAuditAllSttus_xml", "DS002-2019013");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            RequestParts::structured_xml(
                "/api/hmvAuditAllSttus.xml",
                operation,
                &parameters,
                "result",
            ),
            decode_director_auditor_total_compensation_xml_response,
        ))
    }
}
...
pub struct DirectorAuditorTotalCompensationEntry<'a> {
    source: &'a SourceValue,
}

impl<'a> DirectorAuditorTotalCompensationEntry<'a> {
    ...
    /// Returns source field `jan_avrg_mendng_am` when present.
    #[must_use]
    pub fn average_compensation_per_person(&self) -> Option<&SourceValue> {
        self.source.get("jan_avrg_mendng_am")
    }
...
/// Opaque response for physical operation `get_hmvAuditAllSttus_json`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct DirectorAuditorTotalCompensationJsonResponse {
    source: SourceValue,
}

impl DirectorAuditorTotalCompensationJsonResponse {
    ...
    pub fn items(&self) -> impl Iterator<Item = DirectorAuditorTotalCompensationEntry<'_>> + '_ {
        items(&self.source, "list", false).map(DirectorAuditorTotalCompensationEntry::new)
    }
}

fn decode_director_auditor_total_compensation_json_response(
    source: SourceValue,
) -> Result<DirectorAuditorTotalCompensationJsonResponse, ResponseDecodeError> {
    decode_director_auditor_total_compensation_json_response_root(&source, "$".to_owned())?;
    Ok(DirectorAuditorTotalCompensationJsonResponse { source })
}

fn decode_director_auditor_total_compensation_json_response_root(
    value: &SourceValue,
    path: String,
    ...
    object.optional(
        "list",
        decode_director_auditor_total_compensation_json_response_root_list,
    )?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_director_auditor_total_compensation_json_response_root_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_array(
        value,
        path,
        decode_director_auditor_total_compensation_json_response_root_list_item,
    )?;
    Ok(())
}

fn decode_director_auditor_total_compensation_json_response_root_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
...
/// Opaque response for physical operation `get_hmvAuditAllSttus_xml`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct DirectorAuditorTotalCompensationXmlResponse {
    source: SourceValue,
}

impl DirectorAuditorTotalCompensationXmlResponse {
    /// Returns the complete normalized source object.
    #[must_use]
    pub const fn source(&self) -> &SourceValue {
        &self.source
    ...
    /// Iterates the reviewed `$.list[]` item views.
    pub fn items(&self) -> impl Iterator<Item = DirectorAuditorTotalCompensationEntry<'_>> + '_ {
        items(&self.source, "list", true).map(DirectorAuditorTotalCompensationEntry::new)
    }
}

fn decode_director_auditor_total_compensation_xml_response(
    source: SourceValue,
) -> Result<DirectorAuditorTotalCompensationXmlResponse, ResponseDecodeError> {
    decode_director_auditor_total_compensation_xml_response_root(&source, "$".to_owned())?;
    Ok(DirectorAuditorTotalCompensationXmlResponse { source })
}

fn decode_director_auditor_total_compensation_xml_response_root(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    ...
    object.optional(
        "list",
        decode_director_auditor_total_compensation_xml_response_root_list,
    )?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_director_auditor_total_compensation_xml_response_root_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_xml_array(
        value,
        path,
        decode_director_auditor_total_compensation_xml_response_root_list_item,
    )?;
    Ok(())
}

fn decode_director_auditor_total_compensation_xml_response_root_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
