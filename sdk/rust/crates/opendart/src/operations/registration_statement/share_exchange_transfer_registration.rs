...
/// Inputs for logical operation `DS006-2020058`.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ShareExchangeTransferRegistrationInput {
    company_code: String,
    start_date: String,
    end_date: String,
}

impl ShareExchangeTransferRegistrationInput {
    ...
    /// Prepares physical operation `get_extrRs_json`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_json(
        &self,
    ) -> Result<PreparedRequest<ShareExchangeTransferRegistrationJsonResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_extrRs_json", "DS006-2020058");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            RequestParts::structured_json("/api/extrRs.json", operation, &parameters),
            decode_share_exchange_transfer_registration_json_response,
        ))
    }

    /// Prepares physical operation `get_extrRs_xml`.
    ///
    /// # Errors
    /// Returns a stable [`PrepareError`] when supplied input violates canonical OpenAPI constraints.
    pub fn prepare_xml(
        &self,
    ) -> Result<PreparedRequest<ShareExchangeTransferRegistrationXmlResponse>, PrepareError> {
        let operation = OperationIdentity::new("get_extrRs_xml", "DS006-2020058");
        let parameters = self.parameters(operation)?;
        Ok(PreparedRequest::new(
            RequestParts::structured_xml("/api/extrRs.xml", operation, &parameters, "result"),
            decode_share_exchange_transfer_registration_xml_response,
        ))
    }
}
...
pub struct ShareExchangeTransferRegistrationSection<'a> {
    source: &'a SourceValue,
    xml: bool,
}

impl<'a> ShareExchangeTransferRegistrationSection<'a> {
    ...
    pub fn items(&self) -> impl Iterator<Item = ShareExchangeTransferRegistrationEntry<'a>> + '_ {
        items(self.source, "list", self.xml).map(ShareExchangeTransferRegistrationEntry::new)
    }
}

/// Borrowed semantic view over `$.group[].list[]`.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct ShareExchangeTransferRegistrationEntry<'a> {
    source: &'a SourceValue,
}

impl<'a> ShareExchangeTransferRegistrationEntry<'a> {
...
/// Opaque response for physical operation `get_extrRs_json`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct ShareExchangeTransferRegistrationJsonResponse {
    source: SourceValue,
}

impl ShareExchangeTransferRegistrationJsonResponse {
    ...
    pub fn items(&self) -> impl Iterator<Item = ShareExchangeTransferRegistrationSection<'_>> + '_ {
        items(&self.source, "group", false)
            .map(|source| ShareExchangeTransferRegistrationSection::new(source, false))
    }
}

fn decode_share_exchange_transfer_registration_json_response(
    source: SourceValue,
) -> Result<ShareExchangeTransferRegistrationJsonResponse, ResponseDecodeError> {
    decode_share_exchange_transfer_registration_json_response_root(&source, "$".to_owned())?;
    Ok(ShareExchangeTransferRegistrationJsonResponse { source })
}

fn decode_share_exchange_transfer_registration_json_response_root(
    value: &SourceValue,
    path: String,
    ...
    object.optional(
        "group",
        decode_share_exchange_transfer_registration_json_response_root_group,
    )?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_share_exchange_transfer_registration_json_response_root_group(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_array(
        value,
        path,
        decode_share_exchange_transfer_registration_json_response_root_group_item,
    )?;
    Ok(())
}

fn decode_share_exchange_transfer_registration_json_response_root_group_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    let object = ObjectDecoder::new(value, path)?;
    object.optional(
        "list",
        decode_share_exchange_transfer_registration_json_response_root_group_item_list,
    )?;
    Ok(())
}

fn decode_share_exchange_transfer_registration_json_response_root_group_item_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_array(
        value,
        path,
        decode_share_exchange_transfer_registration_json_response_root_group_item_list_item,
    )?;
    Ok(())
}

fn decode_share_exchange_transfer_registration_json_response_root_group_item_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
...
/// Opaque response for physical operation `get_extrRs_xml`.
#[derive(Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize))]
#[cfg_attr(feature = "serde-json", serde(transparent))]
pub struct ShareExchangeTransferRegistrationXmlResponse {
    source: SourceValue,
}

impl ShareExchangeTransferRegistrationXmlResponse {
    /// Returns the complete normalized source object.
    #[must_use]
    pub const fn source(&self) -> &SourceValue {
        &self.source
    ...
    pub fn items(&self) -> impl Iterator<Item = ShareExchangeTransferRegistrationSection<'_>> + '_ {
        items(&self.source, "group", true)
            .map(|source| ShareExchangeTransferRegistrationSection::new(source, true))
    }
}

fn decode_share_exchange_transfer_registration_xml_response(
    source: SourceValue,
) -> Result<ShareExchangeTransferRegistrationXmlResponse, ResponseDecodeError> {
    decode_share_exchange_transfer_registration_xml_response_root(&source, "$".to_owned())?;
    Ok(ShareExchangeTransferRegistrationXmlResponse { source })
}

fn decode_share_exchange_transfer_registration_xml_response_root(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    ...
    object.optional(
        "group",
        decode_share_exchange_transfer_registration_xml_response_root_group,
    )?;
    object.optional("status", decode_source_status)?;
    Ok(())
}

fn decode_share_exchange_transfer_registration_xml_response_root_group(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_xml_array(
        value,
        path,
        decode_share_exchange_transfer_registration_xml_response_root_group_item,
    )?;
    Ok(())
}

fn decode_share_exchange_transfer_registration_xml_response_root_group_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    let object = ObjectDecoder::new_xml(value, path)?;
    object.optional(
        "list",
        decode_share_exchange_transfer_registration_xml_response_root_group_item_list,
    )?;
    Ok(())
}

fn decode_share_exchange_transfer_registration_xml_response_root_group_item_list(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
    decode_xml_array(
        value,
        path,
        decode_share_exchange_transfer_registration_xml_response_root_group_item_list_item,
    )?;
    Ok(())
}

fn decode_share_exchange_transfer_registration_xml_response_root_group_item_list_item(
    value: &SourceValue,
    path: String,
) -> Result<(), ResponseDecodeError> {
