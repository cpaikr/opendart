...
The checked-in `operations` modules are handwritten and use reviewed Rust
product names. Operation inputs, preparation methods, opaque source-backed
response wrappers, and behavior participate in SemVer.
...
use opendart::{
    operations::disclosure::CompanyOverviewInput,
    ApiKey, Client, SourceReply,
};

# #[cfg(feature = "client-reqwest")]
# async fn example() -> Result<(), Box<dyn std::error::Error>> {
let request = CompanyOverviewInput::new("00126380".to_owned())
    .prepare_json()?;
...
match client.execute(&request).await?.reply {
    SourceReply::Success(company) => {
        println!("company name evidence: {:?}", company.legal_name())
    }
...
opaque handwritten response wrapper. The wrapper validates established
structure eagerly, retains the complete normalized `SourceValue`, exposes
reviewed typed accessors, and provides `source()` for additive evidence.
...
Structured response wrappers and shared response, status, metadata, and
`SourceValue` types then implement `serde::Serialize`. Wrapper serialization
delegates to the retained source representation. Source numbers are
...
success-shaped body under HTTP 500 is never decoded into the structured
response wrapper. Callers remain responsible for bounding body collection
before passing the slice to this defensive interpreter.
...
`source_provenance()` identifies the crate version, selected semantic
