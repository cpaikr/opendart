...
func newDriftHTTPFetcher() *HTTPFetcher {
	...
	fetcher.attempts = 1
	// net/http may transparently retry idempotent requests after a failure on a
	// reused connection. Drift trades connection reuse for a literal one-wire-
	// attempt policy; regular synchronization keeps its pooled retry behavior.
