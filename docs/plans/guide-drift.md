...
and performs an OpenAPI-aware semantic comparison. A changed report identifies
affected guide identities, physical operations, and contract locations.
...
- Pass only the schema-validated sanitized report to a separate notification
  job with `issues: write`. That job has no API key and cannot consume raw
  source artifacts.
- Protect issue writing with a default-branch-only GitHub Environment.

On `changed` or `error`, find the open issue containing
`<!-- opendart-guide-drift:v1 -->` and update it, or create one titled
`OpenDART guide semantic drift`. The issue must mention `@sjunepark` and link
the workflow run with the sanitized evidence. `unchanged` creates no issue.
Automation never auto-closes the issue. This marker and issue are distinct from
...
- [ ] Add the scheduled read-only job, artifact upload, and isolated
      deduplicated issue notification.
...
- Every non-success path leaves a machine-readable sanitized result.
